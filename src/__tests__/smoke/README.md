...
2. **Environment Variables**: Set staging/production endpoints
   ...
   export STAGING_USER_POOL_ID=us-east-1_XXXXXXXXX
   export STAGING_USER_POOL_CLIENT_ID=XXXXXXXXXXXXXXXXXXXXXXXXXX
...
- ✅ Correct password policy configured
- ✅ Phone sign-in enabled
- ✅ Correct user pool name
...
PASS  src/__tests__/smoke/staging.smoke.test.ts
  Staging Environment Smoke Tests
    ...
    Cognito User Pool
      ✓ should have user pool accessible (XXXms)
      ✓ should have correct password policy configured (XXXms)
      ✓ should have phone sign-in enabled (XXXms)
    Performance Checks
...
- name: Run smoke tests
  env:
    ...
    STAGING_USER_POOL_ID: ${{ needs.deploy-infrastructure.outputs.user-pool-id }}
    # ... other env vars
...
- [Staging Deployment Guide](../../../infrastructure/STAGING_DEPLOYMENT.md)
