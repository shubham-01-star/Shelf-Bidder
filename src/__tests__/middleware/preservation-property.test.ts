...
function runMiddlewarePipeline(req: NextRequest): NextResponse {
  ...
  if (fs.existsSync(path.join(rootDir, 'src', 'proxy.ts'))) {
    ...
    if (fn) {
      const res = fn(req);
      if (res) {
        if (res.status === 401 || res.status === 204) return res; // Auth failure or OPTIONS preflight
        res.headers.forEach((v: string, k: string) => response.headers.set(k, v));
      }
    }
  }
...
describe('Preservation Property Tests - Middleware Functionality', () => {
  ...
  it('Property: Security headers are set on all responses', () => {
    fc.assert(
      fc.property(
        fc.string().filter(s => /^[a-zA-Z0-9/-]+$/.test(s) && s.length > 0),
        (pathname) => {
          const req = new NextRequest(`${baseUrl}/${pathname}`);
          const res = runMiddlewarePipeline(req);
  ...
  it('Property: Public API routes allow access without authentication', () => {
  ...
  it('Property: Protected API routes reject unauthenticated requests', () => {
    fc.assert(
      fc.property(
  ...
  it('Property: Protected API routes allow authenticated requests (auth_token cookie)', () => {
  ...
  it('Property: Protected API routes allow authenticated requests (Bearer token)', () => {
    const req = new NextRequest(`${baseUrl}/api/protected/data`, {
  ...
  it('Property: Preflight OPTIONS requests return 204 with CORS headers', () => {
  ...
  it('Property: API Routes include CORS headers', () => {
    ...
    req.cookies.set('auth_token', 'valid');
    const res = runMiddlewarePipeline(req);
