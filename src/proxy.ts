...
import { getAuthClaimsFromRequest } from '@/lib/auth/server-auth';
...
const PUBLIC_API_ROUTES = [
  ...
  '/api/brand/auth/refresh',
  '/api/brand/auth/logout',
  '/api/brand/auth/verify',
  '/api/health',
  '/api/images/search',
];
...
export async function proxy(request: NextRequest) {
  ...
  if (pathname.startsWith('/api') && !PUBLIC_API_ROUTES.some(r => pathname.startsWith(r))) {
    try {
      await getAuthClaimsFromRequest(request);
    } catch {
