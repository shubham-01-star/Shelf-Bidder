...
export async function creditEarnings(
  ...
  // Resolve external auth/shopkeeper identifier -> shopkeepers.id (UUID) for FK reference
...
export async function getTransactionHistory(
  ...
  // Resolve external auth/shopkeeper identifier -> shopkeepers.id (UUID) for FK queries
  const shopkeeper = await ShopkeeperOperations.getByShopkeeperId(shopkeeperId);
...
export async function requestPayout(
  ...
  // processPayout expects shopkeepers.id (UUID), not the external-facing identifier
