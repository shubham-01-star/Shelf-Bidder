...
export const CampaignOperations = {
  ...
  async findMatchingCampaigns(
    ...
    const sql = `
      ...
      ORDER BY payout_per_task DESC, created_at ASC
    `;
    const result = await query<CampaignRow>(sql, [minBudget]);
    const now = new Date();
    const campaigns = result.rows
      .map(CampaignMapper.fromRow)
      .filter((campaign) => campaign.start_date <= now && campaign.end_date >= now);
    ...
    const matchedCampaigns = campaigns.filter((campaign) => {
      ...
      const hasMatch = campaign.target_locations.some((targetLocation) => {
